...
export class KeywordResolver {
  ...
  public async clicks(@Parent() { series }: KeywordModel, @Args('input') query: QueryInput): Promise<ScalarValue[]> {
  ...
  public async impressions(@Parent() { series }: KeywordModel, @Args('input') query: QueryInput): Promise<ScalarValue[]> {
    return await this.timescaleService.resolveQuery(GoogleEntity, series, 'impressions', query);
  }

  @ResolveField(() => [ScalarValue], { name: 'ctr' })
  public async ctr(@Parent() { series }: KeywordModel, @Args('input') query: QueryInput): Promise<ScalarValue[]> {
    return await this.timescaleService.resolveQuery(GoogleEntity, series, 'ctr', query);
  }

  @ResolveField(() => [ScalarValue], { name: 'position' })
  public async position(@Parent() { series }: KeywordModel, @Args('input') query: QueryInput): Promise<ScalarValue[]> {
