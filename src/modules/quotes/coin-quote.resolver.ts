...
export class CoinQuoteResolver {
  ...
  public async price(@Parent() { series }: CoinQuoteModel, @Args('input') query: QueryInput): Promise<ScalarValue[]> {
    return await this.timescaleService.resolveQuery(QuotesEntity, series, 'price', query);
  }

  @ResolveField(() => [ScalarValue], { name: 'market_cap' })
  public async market_cap(@Parent() { series }: CoinQuoteModel, @Args('input') query: QueryInput): Promise<ScalarValue[]> {
  ...
  public async volume_24h(@Parent() { series }: CoinQuoteModel, @Args('input') query: QueryInput): Promise<ScalarValue[]> {
